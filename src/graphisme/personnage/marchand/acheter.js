...
export const acheter = (prix, code, id, storeCelestin, storeInventaire, storeJoueurs, dialogueSet, stickerSet, itemCourant, quantite, storeParametre) => {

    if (quantite != '6000000') {
        const poidTotal = storeInventaire.poid + (itemCourant.poid * quantite);

        if (storeInventaire.poidMax >= poidTotal) {
            ...
            if (storeCelestin.argent >= prixReel) {
                if (code != 'test') {
                    if (storeCelestin.codeReduction != 'fini') {
                        ...
                        if (ligneAajouter) {
                            storeInventaire.ajouterQuantiteItem(id, 'quantite', 1 * quantite);
                            storeInventaire.ajouter('poid', itemCourant.poid * quantite);
                        } else {
                            const ligne = {
                                equipe: 0,
