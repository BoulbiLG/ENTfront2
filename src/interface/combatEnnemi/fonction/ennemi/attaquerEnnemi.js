...
export const attaquerEnnemi = (storeEnnemis, storeJoueurs, lexiqueArme, storeCombat, tourSet, strategieEnnemi, strategieEnnemiSet, setJoueurUtilisable, joueurUtilisable, storeEquipe, ennemiEnVie, historique, historiqueSet) => {
    ...
    //console.log('strategieEnnemi : ', strategieEnnemi)
    ...
    for (let i = 0; i < storeEnnemis.ennemi.length; i++) {
        ...
        // CALCUL ATTAQUE
        const calculAttaqueNet = (store, tableau) => {
            ...
            for (const joueur of storeJoueurs) {
                ...
                for (let e = 0; e < tableau.length; e++) {
                    if (tableau[e].nom === store.id) {ligneTableauCible = tableau[e]}
                }
        ...
        */
        ...
        const estPresent = ennemiEnVie.includes(store.id);
                ...
                if (nombre >= min && nombre <= max) {
                    ...
                    if (nombre >= 0 && nombre <= 5) {
                        ...
                        if (tableauMagie[0].sortStatOffensif[0]) {
                            ...
                            calculAttaqueNet(store, tableau);
                        } else {
                        ...
                        if (nombre === 1) {
                            ...
                            if (tableauMagie[0].sortStatAugmente[0]) {
                                ...
                                tourTermine = utiliserSortStat(storeCombat, store, storeJoueurs, tableauMagie[0].sortStatAugmente[0], 'augmente', historique, historiqueSet, storeEnnemis);
                                //calculAttaqueNet();
                            } else {
                            ...
                            if (tableauMagie[0].sortBaisseStat[0]) {
                                ...
                                storeJoueurs.forEach((joueur) => {
                                    if (joueur.nom === tableau[0].cible) {
                                        tourTermine = utiliserSortStat(storeCombat, store, joueur, tableauMagie[0].sortBaisseStat[0], 'baisse', historique, historiqueSet, storeEnnemis);
                                    }
                                });
                            } else {
                            ...
                            if (tableauMagie[0].sortStatus[0]) {
                                ...
                                storeJoueurs.forEach((joueur) => {
                                    if (joueur.nom === tableau[0].cible) {
                                        tourTermine = utiliserSortStat(storeCombat, store, joueur, tableauMagie[0].sortStatus[0], 'status', historique, historiqueSet, storeEnnemis);
                                    }
                                });
                            } else {
